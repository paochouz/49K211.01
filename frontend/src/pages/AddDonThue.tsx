...
export default function AddDonThue() {
  ...
  // Auto mã đơn: lấy số lớn nhất + 1 (tạm dùng localStorage)
  useEffect(() => {
    const lastRaw = localStorage.getItem(STORAGE_KEYS.lastInvoiceNumber);
    const last = lastRaw ? Number(lastRaw) : 0;
    const next = Number.isFinite(last) && last > 0 ? last + 1 : 1;
    setForm((prev) => ({ ...prev, invoiceNumber: next, maDon: formatInvoice(next) }));
  }, []);
    ...
    <div style={containerStyle}>
      <div style={cardStyleWide}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <h2 style={titleStyle}>Tạo đơn thuê trang phục</h2>
          <button
            type="button"
