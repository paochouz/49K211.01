...
import {
  ensureDemoRentInvoices,
  getRentInvoiceByMaDon,
  isDonChuaCoc,
  upsertRentInvoice,
  type StoredRentInvoice,
} from "../lib/rentInvoiceStorage";
...
const seedCustomers: Customer[] = [
  { maKH: "KH000101", tenKH: "Nguyễn Anh Khoa", soDienThoai: "0973729452" },
  { maKH: "KH000102", tenKH: "Nguyễn Văn An", soDienThoai: "0912345678" },
  { maKH: "KH000103", tenKH: "Trần Thị Bình", soDienThoai: "0987654321" },
];
...
function DateField({
  ...
  const inputFieldStyle = disabled ? { ...inputStyle, ...inputDisabledStyle } : inputStyle;

  if (disabled) {
    return (
      <div style={{ position: "relative" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 42px", gap: 8, alignItems: "end" }}>
          <input value={value} readOnly disabled placeholder={placeholder ?? "dd/mm/yyyy"} style={inputFieldStyle} />
          <button
            type="button"
            aria-label="Chọn ngày"
            disabled
            style={{ ...calendarButtonStyle, opacity: 0.55, cursor: "not-allowed" }}
          >
            📅
          </button>
        </div>
...
export default function EditDonThue() {
  const navigate = useNavigate();
  const { maDon: maDonParam } = useParams<{ maDon: string }>();
  const maDonFromRoute = maDonParam ? decodeURIComponent(maDonParam) : "";
  ...
  const [boot, setBoot] = useState<"loading" | "ok" | "missing">("loading");
  ...
  const canEdit = isDonChuaCoc(form.trangThai);

  useEffect(() => {
    ensureDemoRentInvoices();
    const target = maDonFromRoute.trim();
    if (!target) {
      setBoot("missing");
      return;
    }
    const inv = getRentInvoiceByMaDon(target);
    if (!inv) {
      setBoot("missing");
      return;
    }
    applyInvoiceToState(inv);
    setTienCocTouched(true);
    setBoot("ok");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [maDonFromRoute]);

  function applyInvoiceToState(inv: StoredRentInvoice) {
    setForm({
      maDon: inv.maDon,
      invoiceNumber: inv.invoiceNumber,
      khachHang: inv.khachHang,
      hinhThucCoc: inv.hinhThucCoc,
      tienCoc: inv.tienCoc,
      trangThai: inv.trangThai,
      ghiChuGiayTo: inv.ghiChuGiayTo,
    });
  ...
  // Tự động tiền cọc theo hình thức (chỉ khi đơn đang được phép sửa — trạng thái Chưa cọc)
  useEffect(() => {
    if (boot !== "ok" || !canEdit) return;
  ...
  }, [boot, canEdit, form.hinhThucCoc, derived.tongDonThue, form.tienCoc, tienCocTouched]);
  ...
  const filteredCostumes = useMemo(() => {
    ...
    if (!kw) return seedCostumes;
    return seedCostumes.filter((c) => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!canEdit) {
      setIsError(true);
      setMessage(`Đơn thuê ở trạng thái ${form.trangThai || "này"} — không thể chỉnh sửa.`);
      return;
    }
    ...
    const payload: StoredRentInvoice = {
      maDon: form.maDon,
    ...
    return (
      <div style={containerStyle}>
        <div style={cardStyleWide}>
          <p style={{ color: "#64748b", margin: 0 }}>Đang tải hóa đơn…</p>
        </div>
      </div>
    );
  }
  ...
  if (boot === "missing") {
    return (
      <div style={containerStyle}>
        <div style={cardStyleWide}>
          <h2 style={titleStyle}>Chỉnh sửa đơn thuê</h2>
          <p style={{ color: "#64748b" }}>Không tìm thấy hóa đơn {maDonFromRoute || "(chưa có mã)"}.</p>
          <button type="button" style={{ ...secondaryButtonStyle, marginTop: 12 }} onClick={() => navigate(-1)}>
            Quay lại
          </button>
        </div>
      </div>
      ...
      <div style={cardStyleWide}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={groupStyle}>
            ...
            <div style={{ marginTop: 6, display: "flex", gap: 14, flexWrap: "wrap", opacity: canEdit ? 1 : 0.75 }}>
              <label style={{ ...radioLabelStyle, cursor: canEdit ? "pointer" : "not-allowed" }}>
                <input
                  type="radio"
                  value="GIAY_TO"
                  disabled={!canEdit}
          ...
          <div style={groupStyle}>
            ...
            <input value={form.maDon} disabled style={{ ...inputStyle, ...inputDisabledStyle }} />
          </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "end" }}>
              <div>
                <input
                  ...
                  style={{ ...inputStyle, ...(fieldReadonly ? inputDisabledStyle : {}) }}
                />
              </div>
              <button
                type="button"
                  ...
                  <div key={item.id} style={itemCardStyle}>
                    ...
                    <div style={grid2Style}>
                      <div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "end" }}>
                          ...
                          <button
                            type="button"
                            disabled={!canEdit}
                            onClick={() => openCostumeModal(item.id)}
                            style={{
                              ...secondaryButtonStyle,
                              opacity: canEdit ? 1 : 0.55,
                              cursor: canEdit ? "pointer" : "not-allowed",
                      ...
                      <div>
                        ...
                        <input value={item.donGia} disabled style={{ ...inputStyle, ...inputDisabledStyle }} />
                      </div>
                    </div>
                    ...
                    <div style={grid3Style}>
                      ...
                      <div>
                        ...
                        <input value={row?.thanhTien ?? 0} disabled style={{ ...inputStyle, ...inputDisabledStyle }} />
                      </div>
                    </div>
          ...
          <div style={groupStyle}>
            ...
            <input value={derived.phiTraTre} disabled style={{ ...inputStyle, ...inputDisabledStyle }} />
          </div>

          {/* 10. Tổng đơn thuê* */}
          <div style={groupStyle}>
            <label style={labelStyle}>Tổng đơn thuê *</label>
            <input value={derived.tongDonThue} disabled style={{ ...inputStyle, ...inputDisabledStyle }} />
          </div>
          ...
          <div style={groupStyle}>
            ...
            <input
              type="number"
              value={form.tienCoc}
              disabled={!canEdit || form.hinhThucCoc === "GIAY_TO"}
              onChange={(e) => {
                setTienCocTouched(true);
              ...
              style={{
                ...inputStyle,
                ...((!canEdit || form.hinhThucCoc === "GIAY_TO") ? inputDisabledStyle : {}),
              }}
            />
          </div>
          ...
          <div style={groupStyle}>
            ...
            <input value={derived.tienPhaiTra} disabled style={{ ...inputStyle, ...inputDisabledStyle }} />
          </div>

          {/* 12. Trạng thái đơn */}
          <div style={groupStyle}>
            <label style={labelStyle}>Trạng thái đơn</label>
            <input value={form.trangThai} readOnly disabled style={{ ...inputStyle, ...inputDisabledStyle }} />
          </div>
          ...
          {!canEdit && form.trangThai && (
            <div style={warningBannerStyle} role="status">
              Đơn thuê ở trạng thái <strong>{form.trangThai}</strong> — không thể chỉnh sửa.
            </div>
          )}
          ...
          <div style={footerActionsStyle}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                fontWeight: 600,
                border: canEdit ? "none" : "1px solid #60A5FA",
                background: canEdit ? "#2563EB" : "#BFDBFE",
                color: canEdit ? "#fff" : "#1e40af",
